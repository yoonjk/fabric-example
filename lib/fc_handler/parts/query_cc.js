...
module.exports = function (logger) {
	...
	*/
	query_cc.query_chaincode = function (obj, options, cb) {
		logger.debug('[fca] Querying Chaincode: ' + options.cc_function + '()');
		...
		logger.debug('[fca] Sending query req:', request);
		...
		channel.queryByChaincode(request).then(function (response_payloads) {
			...
			if (formatted.parsed == null) {
				logger.debug('[fca] Query parsed response is empty:', formatted.parsed);
			}
			if (formatted.error) {
				logger.debug('[fca] Query response is an error:', formatted.error);
			}

			// --- response looks good --- //
			else {
				logger.debug('[fca] Successful query transaction.');
			}
			if (cb) return cb(formatted.error, formatted);
		}).catch(function (err) {
			logger.error('[fca] Error in query catch block', typeof err, err);
	...
	function format_query_resp(peer_responses) {
		...
		for (var i in peer_responses) {
			...
			if (last != null) {									//check if all peers agree
				if (last !== as_string) {
					logger.warn('[fca] warning - some peers do not agree on query', last, as_string);
			...
			try {
				...
				logger.debug('[fca] Peer Query Response - len:', as_string.length, 'type:', typeof as_obj);
				if (ret.parsed === null) ret.parsed = as_obj;	//store the first one here
			} catch (e) {
				if (known_sdk_errors(as_string)) {
					logger.error('[fca] query resp looks like an error:', typeof as_string, as_string);
					ret.parsed = null;
					ret.error = as_string;
				} else if (as_string.indexOf('premature execution') >= 0) {
					logger.warn('[fca] query not successful, waiting on chaincode to start:', as_string);
					ret.parsed = null;
					ret.error = as_string;
				} else {
					logger.warn('[fca] warning - query resp is not json, might be okay:', typeof as_string, as_string);
