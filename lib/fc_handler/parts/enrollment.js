...
module.exports = function (logger) {
	...
	*/
	...
	enrollment.enroll = function (options, cb) {
		...
		// logger.info('[fca] Going to enroll', debug);
		...
		FabricClient.newDefaultKeyValueStore({
			...
			//
			// // --- Success --- //
			logger.debug('[fca] Successfully got enrollment ');
			...
			// --- Failure --- //
			logger.error('[fca] Failed to get enrollment ' + options.uuid, err.stack ? err.stack : err);
		...
		return client.getUserContext(options.enroll_id, true).then((user) => {
			if (user && user.isEnrolled()) {
				if (user._mspId !== options.msp_id) {										//if they don't match, this isn't our user! can't use it
					logger.warn('[fca] The msp id in KVS does not match the msp id passed to enroll. Need to clear the KVS.', user._mspId, options.msp_id);
					common.rmdir(options.kvs_path);											//delete it
					logger.error('[fca] MSP in KVS mismatch. KVS has been deleted. Restart the app to try again.');
					process.exit();															//this is terrible, but can't seem to reset client._userContext
				} else {
					logger.info('[fca] Successfully loaded enrollment from persistence');	//load from KVS if we can
					return user;
				}
			} else {
				...
				logger.debug('enroll id: "' + options.enroll_id);
				...
				return ca_client.enroll({
					...
					// Store Certs
					logger.info('[fca] Successfully enrolled user \'' + options.enroll_id + '\'');
					...
					// Send Errors
					logger.error('[fca] Failed to enroll and persist user. Error: ' + err.stack ? err.stack : err);
	...
	*/
	...
	enrollment.enrollWithAdminCert = function (options, cb) {
		...
		logger.info('[fca] Going to enroll with admin cert! ', debug);
		// Make eCert kvs (Key Value Store)
		FabricClient.newDefaultKeyValueStore({
			...
			// --- Success --- //
			logger.debug('[fca] Successfully got enrollment ' + options.uuid);
			if (cb) cb(null, { client: client, channel: channel, submitter: submitter });
			return;
