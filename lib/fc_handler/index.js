//-------------------------------------------------------------------
// Fabric Client Adapter - Wrapper library for the Hyperledger Fabric Client SDK
//-------------------------------------------------------------------
...
module.exports = function (g_options, logger) {
	...
	// Install Chaincode
	fca.install_chaincode = function (obj, options, cb_done) {
		deploy_cc.install_chaincode(obj, options, cb_done);
	};

	// Instantiate Chaincode
	fca.instantiate_chaincode = function (obj, options, cb_done) {
		deploy_cc.instantiate_chaincode(obj, options, cb_done);
	};

	// Upgrade Chaincode
	fca.upgrade_chaincode = function (obj, options, cb_done) {
	...
	*/
	fca.invoke_chaincode = function (obj, options, cb_done) {
	...
	*/
	fca.query_chaincode = function (obj, options, cb_done) {
	...
	fca.enroll = function (options, cb_done) {
	...
	fca.enrollWithAdminCert = function (options, cb_done) {
	...
	// ------------------------------------------------------------------------
	// Get Block Data
	fca.query_block = function (obj, options, cb_done) {
	...
	fca.query_channel_members = function (obj, options, cb_done) {
	...
	*/
	fca.query_channel = function (obj, options, cb_done) {
		query_peer.query_channel(obj, options, function (err, resp) {
			if (err != null) {											//looks like an error with the request
				if (ha.switch_peer(obj, options) == null) {				//try another peer
					logger.info('Retrying query on different peer');
					fca.query_channel(obj, options, cb_done);
				} else {
					if (cb_done) cb_done(err, resp);					//out of peers, give up
				}
			} else {													//all good, pass resp back to callback
	...
	fca.query_installed_cc = function (obj, options, cb_done) {
		query_peer.query_installed_cc(obj, options, cb_done);
	};

	// get list of instantiated cc's
	fca.query_instantiated_cc = function (obj, options, cb_done) {
		query_peer.query_instantiated_cc(obj, options, cb_done);
	};

	// get list of channels
	fca.query_list_channels = function (obj, options, cb_done) {
