...
class FabricClient {
  ...
  */
  ...
  newChannel() {
    return this.fabric_client.newChannel(this.channelId);     
  }


  newPeer() {
    const peer = this.fabric_client.newPeer(this.peer.url,{
                    'ssl-target-name-override': this.peer0Name,
                    'pem': peer.tlsCaCerts.pem
                 });
